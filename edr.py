...
# sample code to limit file uploads, by only reading the number designated by the max_lines variable. 
# MAX_LINES = 2
...
def umagic(df_edr):
    st.title('EDR File Visualisation')
    
    if df_edr is not None:
        st.warning('No file has been uploaded')
    
    else:
        # columns = list(df_edr.columns)
